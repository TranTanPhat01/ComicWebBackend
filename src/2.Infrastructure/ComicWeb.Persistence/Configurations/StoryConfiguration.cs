...
public sealed class StoryConfiguration : IEntityTypeConfiguration<Story>
{
    public void Configure(EntityTypeBuilder<Story> b)
    {
        ...
        b.Property(x => x.AuthorName).HasMaxLength(250);
        b.Property(x => x.Status).HasConversion<string>().HasMaxLength(32).IsRequired();
        b.Property(x => x.Version).IsConcurrencyToken();
        
        b.HasQueryFilter(x => x.DeletedAt == null);
        b.HasIndex(x => x.Slug).IsUnique().HasFilter("\"DeletedAt\" IS NULL");
        b.HasIndex(x => new { x.Status, x.PublishedAt });
        b.HasIndex(x => new { x.ScheduledAt, x.Status });

        // Trigram indexes for optimized full text & substring search
        b.HasIndex(x => x.Title)
         .HasMethod("gin")
         .HasOperators("gin_trgm_ops");

        b.HasIndex(x => x.AuthorName)
         .HasMethod("gin")
         .HasOperators("gin_trgm_ops");
    }
}
